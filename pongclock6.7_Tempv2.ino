...
############################################################################
Code Mod based on v6.7
Removed Sync to xxxx
...
v1
On Main Clock, alternate Day / Date with Temp



To Do
Will not run when comment in dht function!

Sketch uses 25,616 bytes (79%) of program storage space. Maximum is 32,256 bytes.
Global variables use 1,615 bytes (78%) of dynamic memory, leaving 433 bytes for local variables. Maximum is 2,048 bytes.
Low memory available, stability problems may occur.

with DH.begin out
Sketch uses 25,572 bytes (79%) of program storage space. Maximum is 32,256 bytes.
Global variables use 1,615 bytes (78%) of dynamic memory, leaving 433 bytes for local variables. Maximum is 2,048 bytes.
Low memory available, stability problems may occur.


PIR Flash LED13 doesn't work, use LED 1/2 on Shield

how to get PIR timer to stay on longer
...
#include <DHT.h>
#include <Timer.h>                   // For reading Temp Sensor every xx mins
...
#define DHTPIN 7                        // Sensor on Pin "D7"
...
Button buttonbrightness = Button(7,BUTTON_PULLDOWN);    // Setup button D brightness quick adj (using button library)
...
//int displaytemp = LOW;                    // Toggle for displaying Temp
//int timeon = 200;                          // how long to stay on
//int counter = 1;
//int display_update = 1;                   // update display?
...
Timer t;                                 // Read Temp Sensor every x minutes

int temp = 0.;                           // only 2 digits to display so no float
int rtc[7];                              // Holds real time clock output
byte clock_mode = 4;                     // Default clock mode. Default = 4 (normal)
...
//  pinMode(sync30sec,INPUT);
  ...
  t.every(900000, getTemp);               // Go Get Temp every 15 Mins (15 * 60 * 1000)
...
// Check Temp Initially
  ...
  Serial.println("Starting up Clock now.....");
  Serial.println("");
...
void normal() {
  ...
  char temp_date[16]; // save the date
  byte mins = 100; //mins
  byte secs = rtc[0]; //seconds
  byte old_secs = secs; //holds old seconds value - from last time seconds were updated o display - used to check if seconds have changed
  int displaytemp = LOW;                    // Toggle for displaying Temp
  ...
  while (run_mode()){ 
    ...
    //if time to change bottom row

    counter = counter + 1;

    if((displaytemp == 1) && (counter >= timeon)) {
        displaytemp = 0; // Display Temp
        //Serial.println(F("Back to Temp"));
        display_update = 1; // Flag to update display
        counter = 0; // 
      }
      else if ((displaytemp == 0) && (counter >= timeon)) {
        displaytemp = 1; // Back to Date
        //Serial.println(F("Back to Date"));
        display_update = 1; // Flag to update display
        counter = 0;
      }
    ...
    if (displaytemp) { 
            ...
            itoa (temp, buffer, 10); // convert temperatire to string
            textchar[8] = 'T';
            textchar[9] = 'm';
            textchar[10] = 'p';
            textchar[11] = ' ';
            ...
            textchar[8] = temp_date[0];
            textchar[9] = temp_date[1];
            textchar[10] = temp_date[2];
            textchar[11] = temp_date[3];
            textchar[12] = temp_date[4];
            textchar[13] = temp_date[5];
            textchar[14] = temp_date[6];
            textchar[15] = temp_date[7];
            }   

    //Serial.println(temp_date); 
    //Serial.print("Counter = ");
    //Serial.print(counter); 
    //Serial.print(" - Display Flag #");
    //Serial.println(displaytemp);
    
    if (display_update == HIGH) {
      //Bottom line needs updating
      for(byte x=0; x<8 ; x++) {
        ht1632_putchar( x * 6, 8, textchar[x+8]); //bottom line
      } 
...
void switch_mode() {
  ...
  char* modes[] = {
    "Slide", "Pong", "Digits", "Words", "Normal", "Setup"};
...
void setup_menu() {
  ...
  char* set_modes[] = { "Go Back", "Random", "24 Hour","Set Clk", "Bright","DST Adj" }; 
  if (ampm == 0) { 
    set_modes[2] = ("12 Hour"); 
  }
